...
func TestISOParse(t *testing.T) {
	...
	isostruct2 := NewISOStruct("spec1987.yml", true)
	parsed2, err := isostruct2.Parse(isomsg)
	if err != nil {
		fmt.Println(err)
		t.Errorf("parse iso message failed")
	}

	isomsgUnpacked2, err := parsed2.ToString()
	if err != nil {
		fmt.Println(err)
		t.Errorf("failed to unpack valid isomsg")
	}
	if isomsgUnpacked2 != isomsg {
		t.Errorf("%s should be %s", isomsgUnpacked2, isomsg)
	}
}
