...
func (e *ElementsType) SetElements(Elements map[int64]string) {
	e.elements = Elements
}
...
func NewISOStruct(filename string, secondaryBitmap bool) IsoStruct {
	// NEW : disini consume banyak memory karna penggunaan map dan load file dari SpecFromFile.
	// jadi dibuat sekali load file saja
	if loadedIsoStruct.Spec.fields != nil {
		return loadedIsoStruct
	}
	...
	loadedIsoStruct = IsoStruct{Spec: spec, Mti: mti, Bitmap: bitmap, Elements: elements}
	return loadedIsoStruct
}
